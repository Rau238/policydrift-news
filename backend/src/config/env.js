...
// Repo root .env first, then backend/.env overrides (optional second file)
...
export const env = {
  ...
  // Prefer API_PORT in shared root .env so PORT= is free for Next.js when dotenv-cli loads the same file.
  ...
  RSS_FEED_URLS: (process.env.RSS_FEED_URLS || '')
    ...
    .map((s) => s.trim())
    .filter(Boolean),
  /** 0 = import every item returned by feeds; else cap total rows per ingest run (newest first). */
  RSS_MAX_ITEMS: parseInt(process.env.RSS_MAX_ITEMS || '0', 10),
  /**
   * When the RSS item has no image, fetch the article URL and parse og:image / twitter:image.
   * Set to "false" to skip (faster ingest; more stories use the fallback image).
   */
  ...
  /** RSS ingest cron: every N minutes (1 = aggressive; 5–30 is gentler on sources). */
  ...
  /** Public site origin (no path), e.g. https://policydrift.example — used to build default story image fallback. */
